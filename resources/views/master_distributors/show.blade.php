...
<div class="ctmr-box mt-4">
 ...
 <!-- Activity Log Panel -->
    ...
    if($field == 'billing_country') {
        $oldValue = \App\Models\Country::find($oldValue)?->country_name ?? $oldValue;
        $newValue = \App\Models\Country::find($newValue)?->country_name ?? $newValue;
    }

    // STATE
    if($field == 'billing_state') {
        $oldValue = \App\Models\State::find($oldValue)?->state_name ?? $oldValue;
        $newValue = \App\Models\State::find($newValue)?->state_name ?? $newValue;
    }

    // DISTRICT
    if($field == 'billing_district') {
        $oldValue = \App\Models\District::find($oldValue)?->district_name ?? $oldValue;
        $newValue = \App\Models\District::find($newValue)?->district_name ?? $newValue;
    }

    // CITY
    if($field == 'billing_city') {
        $oldValue = \App\Models\City::find($oldValue)?->city_name ?? $oldValue;
        $newValue = \App\Models\City::find($newValue)?->city_name ?? $newValue;
    }

    // PINCODE
    if($field == 'billing_pincode') {
        $oldValue = \App\Models\Pincode::find($oldValue)?->pincode ?? $oldValue;
        $newValue = \App\Models\Pincode::find($newValue)?->pincode ?? $newValue;
    }
...
<td class="new-col">
    ...
    $displayValues = collect($displayValues)
                        ->only($allowedFields)
                        ->toArray();
...
<div class="fields-grid">
    ...
    if($key == 'billing_country') {
        $value = \App\Models\Country::find($value)?->country_name ?? $value;
    }

    if($key == 'billing_state') {
        $value = \App\Models\State::find($value)?->state_name ?? $value;
    }

    if($key == 'billing_district') {
        $value = \App\Models\District::find($value)?->district_name ?? $value;
    }

    if($key == 'billing_city') {
        $value = \App\Models\City::find($value)?->city_name ?? $value;
    }

    if($key == 'billing_pincode') {
        $value = \App\Models\Pincode::find($value)?->pincode ?? $value;
    }

    if($key == 'beat_id') {
        $value = \App\Models\Beat::find($value)?->beat_name ?? $value;
    }

    if($key == 'supervisor_id') {
        $value = \App\Models\User::find($value)?->name ?? $value;
    }

    if($key == 'sales_executive_id') {

        $ids = json_decode($value, true);

        if(is_array($ids)) {

            $names = \App\Models\User::whereIn('id', $ids)
                        ->pluck('name')
                        ->toArray();
