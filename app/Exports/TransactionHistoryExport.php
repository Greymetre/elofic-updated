...
class TransactionHistoryExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping
{
    ...
    {
        return ['Transaction Id', 'Date', 'Customer Id', 'Firm Name', 'Contact Person', 'Parent Code', 'Parent Id', 'Parent Name', 'Mobile Number', 'City', 'District', 'State', 'Branch', 'Div', 'Coupon Code', 'Sub Category', 'Prodcut Id', 'Prodcut Name', 'Scheme Name', 'Point', 'Emp Code', 'User Name'];
    }
    ...
    public function map($data): array
    {
        ...
        return [
            ...
            $data['coupen_code'],
            $data['scheme']['product']['subcategories']['subcategory_name'],
            $data['scheme']['product']['id'],
            $data['scheme']['product']['product_name'],
            $scheme_details->scheme->scheme_name,
            ($scheme_details) ? $scheme_details->points: '',
            implode(',', $empcode_arr),
            implode(',', $employee),
        ];
    }
