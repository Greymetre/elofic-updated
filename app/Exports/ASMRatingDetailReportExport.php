...
class ASMRatingDetailReportExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithEvents
{
    ...
    public function map($query): array
    {
        ...
        return [
            ...
            $query['email'] ?? '-',
            $query['userinfo'] ? $query['userinfo']['emergency_number'] : '-',
            $query['mobile'] ?? '-',
            $query['location'] ?? '-',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->mergeCells('T1:W1');
                $event->sheet->mergeCells('X1:AA1');
                $event->sheet->mergeCells('AB1:AE1');
                $event->sheet->mergeCells('AF1:AJ1');
                $event->sheet->mergeCells('AK1:AO1');
                $event->sheet->mergeCells('AP1:AT1');
                $event->sheet->mergeCells('AU1:AY1');
                $event->sheet->mergeCells('BA1:BD1');
