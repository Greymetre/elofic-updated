...
class CheckIn extends Model
{
    ...
    'billing_address' )->with(['city', 'district', 'beat']) ->find($this->entity_id),
    ...
    {
        return $this->belongsTo(MasterDistributor::class, 'entity_id');
    }

    public function secondaryCustomer()
    {
        return $this->belongsTo(SecondaryCustomer::class, 'entity_id');
    }


    public function placedOrders()
{
    $checkinDateTime = $this->checkin_date . ' ' . $this->checkin_time;

    $checkoutDateTime = (
        $this->checkout_date &&
        $this->checkout_time
    )
        ? $this->checkout_date . ' ' . $this->checkout_time
        : now();
    ...
    switch ($this->entity_type) {
        ...
        case 'distributor':
            $query->where('seller_id', $this->entity_id);
            break;

        case 'secondary_customer':
            $query->where('buyer_id', $this->entity_id);
            break;

        case 'customer':
            $query->where('buyer_id', $this->customer_id);
            break;
    }
