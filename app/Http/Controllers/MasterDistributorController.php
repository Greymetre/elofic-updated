...
{
    if ($request->ajax()) {
        $query = MasterDistributor::with(['city', 'state'])
        ->select([
            'id',
            'distributor_code',
            'plant',
        ...
        if ($request->filled('sales_executive_id')) {
            ...
            $query->whereJsonContains(
                'sales_executive_id',
                (int) $request->sales_executive_id
            );
        }
...
->addColumn('billing_city_name', function ($row) {
    return $row->city?->city_name ?? '-';
})

->addColumn('billing_state_name', function ($row) {
...
// ->rawColumns(['beat_route'])
    ...
    public function create()
    {
        ...
        $beats = Beat::where('active', 'Y')
                  ->orderBy('beat_name')
                  ->pluck('beat_name', 'id');
        $branches = Branch::where('active', 'Y')
                    ->pluck('branch_name', 'branch_name');

        // dd($users->toArray());

        return view('master_distributors.create_edit', compact('distributor', 'users','beats','branches'));
    }
   ...
   public function store(Request $request)
{
    ...
    if (!$request->has('same_as_billing') || $request->same_as_billing != 1) {
        $rules += [
            // 'shipping_addresses'   => 'required|string',
    ...
    $data['billing_district'] = $request->district_id;
    $data['billing_city']     = $request->city_id;
    $data['billing_pincode']  = $request->pincode_id;
...
$data['credit_days'] = $request->input('credit_days') !== null 
    ...
    // dd($distributor,$users,$beats,$branches);
        return view('master_distributors.create_edit', compact('distributor', 'users', 'beats', 'branches'));
    }
    ...
    if ($request->hasFile('documents')) {
     ...
     $changesOld = [];
        ...
        foreach ($data as $key => $value) {
            ...
            // skip heavy/unnecessary fields
            if (in_array($key, ['documents', 'shop_image', 'profile_image', 'cancelled_cheque', 'mou_file'])) {
                continue;
            }
    ...
    public function show($id)
{
    $distributor = MasterDistributor::with([
        // 👇 Orders with deep relations
        'orders.orderdetails.products',
        'orders.orderdetails.productdetails',
        'orders.statusname',
        'orders.executive',
    ])->findOrFail($id);
     $totalOrderValue = $distributor->orders->sum('grand_total');

    $totalOrderQty = $distributor->orders->sum(function ($order) {
        return $order->orderdetails->sum('quantity');
    });

    $lastOrderDate = $distributor->orders->max('order_date');

   $activities = ActivityLog::where('module_id', $distributor->id)
    ->where(function ($q) use ($distributor) {
        $q->where('customer_type', $distributor->type) // MECHANIC
          ->orWhere('customer_type', 'MASTER_DISTRIBUTER'); // order logs
    })
    ->latest()
    // ->limit(20)
    ->get();
...
if ($request->filled('billing_city')) {
    ...
    try {
        // ✅ MM/DD/YYYY format (tumhara current payload)
        ...
        // fallback (agar kabhi format change ho jaye)
        $start = Carbon::parse($request->start_date)->startOfDay();
        $end   = Carbon::parse($request->end_date)->endOfDay();
    }

    // ✅ Safety (ulta ho to swap)
    if ($start > $end) {
        [$start, $end] = [$end, $start];
    }
...
public function import(Request $request)
{
    ...
    try {
        ...
        return redirect()
            ->route('master-distributors.index')
            ->with('success', "Successfully imported $count distributors!");
        ...
        foreach ($failures as $failure) {
            ...
            $errors[] = [
                ...
                'errors' => implode(', ', $failure->errors()), // message
