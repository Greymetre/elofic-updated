...
#tab_logic td:nth-child(6) {
    ...
    {{ old('type', $orders->customer_type ?? '') == 'MECHANIC' ? 'selected' : '' }}>
    MECHANIC
</option>

<option value="GARAGE"
    {{ old('type', $orders->customer_type ?? '') == 'GARAGE' ? 'selected' : '' }}>
    GARAGE
</option>

<option value="RETAILER"
    {{ old('type', $orders->customer_type ?? '') == 'RETAILER' ? 'selected' : '' }}>
    RETAILER
</option>

<option value="WORKSHOP"
    {{ old('type', $orders->customer_type ?? '') == 'WORKSHOP' ? 'selected' : '' }}>
    WORKSHOP
</option>

<option value="DISTRIBUTOR"
    {{ old('type', $orders->customer_type ?? '') == 'DISTRIBUTOR' ? 'selected' : '' }}>
    DISTRIBUTOR
</option>
                            </select>
                        </div>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6" id="retailer_div" style="display:none;">
                           <label class="col-form-label">
                              Retailer <span class="text-danger">*</span>
                           </label>
                           ...
                           <select class="form-control select2"
                              ...
                              @if($orders->retailer_id && $orders->retailers)
                                    <option value="{{ $orders->retailer_id }}" selected>
                                       {{ $orders->retailers->shop_name }}
                                    </option>
                              @endif
                           </select>
                           ...
                           <div class="address-card mt-2">
                              <div class="address-title">
                                    <i class="material-icons">store</i> Retailer Address
                              </div>

                              <div id="retailer_address_div" class="address-text">

                                    @if($orders->retailers)
                                       {{ $orders->retailers->address_line ?? '' }},
                                       {{ $cities[$orders->retailers->city_id] ?? '' }},
                                       {{ $districts[$orders->retailers->district_id] ?? '' }},
                                       {{ $states[$orders->retailers->state_id] ?? '' }} -
                                       {{ $pincodes[$orders->retailers->pincode_id] ?? '' }}
                                    @else
                                       Select retailer to view address
                                    @endif
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="addr0" value="1">
                                            ...
                                            <td><input type="number" name="orderdetail[0][price]"
                                                    class="form-control price rowchange" readonly></td>
...
$(document).ready(function () {
    ...
    ============================== */
    $('#buyer_id, #seller_id, #retailer_id').on('select2:open', function () {
   ...
   //       $('#seller_div, #de_dis, #retailer_div').hide();
   //    }
...
$('#type').on('change', function () {
    ...
    if (!firstLoad) {
        ...
        $('#seller_id')
            .val(null)
            .empty()
            .append('<option value="">Select Dealer / Distributor</option>')
            .trigger('change');
        ...
        $('#retailer_id')
            .val(null)
            .empty()
            .append('<option value="">Select Retailer</option>')
            .trigger('change');

        // reset addresses
        $('.buyer_address').html('Select customer to view address');

        $('#customer_address_div').html(
            'Select distributor to view address'
        );

        $('#retailer_address_div').html(
            'Select retailer to view address'
        );
    }
    ...
    if (!firstLoad) {
         ...
         // reload dropdown data after reset
    ...
    $('#buyer_id').on('select2:select', function (e) {
      ...
      $('#retailer_id').on('select2:select', function (e) {
         ...
         let address = `
            ...
            ${window.cities?.[addr.city_id] || ''},
            ${window.districts?.[addr.district_id] || ''},
            ${window.states?.[addr.state_id] || ''} -
            ${window.pincodes?.[addr.pincode_id] || ''}
         `;

         $('#retailer_address_div').html(address);
      });
...
function preloadDropdown(selector, type) {
    ...
    $.get("{{ route('getCustomerDataSelect') }}", {
        ...
        (data.results || []).forEach(item => {
            ...
            // prevent duplicate selected option
