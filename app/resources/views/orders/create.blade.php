 ...
 $('#storeOrderData18').on('submit', function(e){
   ...
   //  e.preventDefault();
    ...
    <tr>
        ...
        <td>
            <input type="number" name="orderdetail[${rowCount}][mrp]" class="form-control price rowchange" readonly >
        </td>
...
function updateRowNumbers(){
    ...
    $('#tab_logic tbody tr').each(function(index){
        ...
        $(document).on('change', '.subcategory-select', function() {
            ...
            $.get("{{ route('getProductsBySubcategory') }}", {
                ...
                $.each(res.products || [], function(i, p) {
                    html += `<option value="${p.id}" data-hsn="${p.price ?? 0}">
        ...
        $('#type').on('change', function() {
            ...
            else if (type === 'GARAGE' || type === 'WORKSHOP') {
            ...
            $('#retailer_id').val(null).trigger('change');
        ...
        function initRetailerDropdown() {
            ...
            $('#retailer_id').select2({
                ...
                $('#retailer_address_div')
                    .text(e.params.data.full_address || '');

            }).on('select2:unselect', function() {

                $('#retailer_address_div').text('');
